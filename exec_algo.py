...
def command_nrj(options):
    command1 = '/opt/cpu_monitor/cpu_monitor.x --csv --quiet --redirect -- '+ options['filename']+' '+options['size1']+' '+options['size2']+' '+options['size3']+' '+options['num_thread']+' 100'+' '+options['dim1']+' '+options['dim2']+' '+options['dim3']
    return command1

def execute_nrj(bash_command, timeout):
    try:
        output = check_output(bash_command, timeout=timeout, shell=True)
        parse_output = 'python test_nrj.py $(ls -t1 ./*.csv | head -n1)'
