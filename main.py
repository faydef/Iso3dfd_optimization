...
def cmdLineParsing():
    ...
    parser.add_argument(
        "--n1",
        help="problem size along first axis default:256",
        default=Defaultn1,
        type=int,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--nt",
        help="maximum number of thread default:32",
        default=Defaultnthread,
        type=int,
    )
    parser.add_argument(
        "--m",
    ...
    if args.m == "firework":
        parser.add_argument(
            "--n",
            help="number of particles/ants/fireworks on each iteration default:100 for population,5 for firework",
            default=Defaultnfirework,
            type=int,
        )
    else:
        parser.add_argument(
            "--n",
            help="number of particles/ants/fireworks on each iteration default:100/5 for firework",
            default=Defaultnumberpopulation,
            type=int,
        )
    parser.add_argument(
        "--rho",
        help="evaporation_rate for ant colonies default:0.8",
        default=Defaultalpha,
        type=np.float64,
    )
    parser.add_argument(
        "--alpha",
        help="value of exponent for ant colonies default:0.25",
        default=Defaultrho,
        type=np.float64,
    )
    parser.add_argument(
        "--q",
        help="quantity if pheromons deposed for ant colonies default:1",
        default=DefaultQ,
        type=np.float64,
    )
    parser.add_argument(
        "--c1",
        help="particle speed for PSO default:0.5",
        default=Defaultc1,
        type=np.float64,
    )
    parser.add_argument(
        "--c2",
        help="group speed for PSO default:0.5",
        default=Defaultc2,
        type=np.float64,
    )
    parser.add_argument(
        "--w",
        help="particle inertia for PSO default:",
        default=Defaultw,
        type=np.float64,
    )
    parser.add_argument(
        "--t",
        help="duration before timeout default:30",
    ...
    parser.add_argument(
        "--ts",
        help="ts : total number of sparks generated by n fireworks default:50",
        default=Defaultts,
        type=int,
    )
    parser.add_argument(
        "--amp",
        help="maximum explosion amplitude default:60",
        default=Defaultamp,
        type=np.float64,
    )
    parser.add_argument(
        "--ngs",
        help="ngs : total number of sparks generated by gaussian distribution default:5",
        default=Defaultngs,
        type=int,
    )
    ...
    if args.m not in ("ACO", "Tree_ACO", "PSO", "firework"):
        sys.exit("Error: local method must be in: [ACO, Tree_ACO, PSO,firework]!")
