...
def cmdLineParsing():
    ...
    parser.add_argument("--i", help="maximum nb of iterations",
                        default=DefaultIterMax, type=int)
    parser.add_argument("--n1", help="problem size along first axis default:256",
                        default=Defaultn1, type=int)
    parser.add_argument("--n2", help="problem size along first axis default:256",
                        default=Defaultn2, type=int)
    parser.add_argument("--n3", help="problem size along first axis default:256",
                        default=Defaultn3, type=int)
    parser.add_argument("--nt", help="maximum number of thread default:32",
                        default=Defaultnthread, type=int)
    parser.add_argument(
        "--m", help="population based method (ACO, Tree_ACO, PSO) default:ACO", default=DefaultMethod)
    parser.add_argument("--n", help="number of particles/ants on each iteration default:100",
                        default=Defaultnumberpopulation, type=int)
    parser.add_argument("--rho", help="evaporation_rate for ant colonies default:0.8",
                        default=Defaultalpha, type=np.float64)
    parser.add_argument("--alpha", help="value of exponent for ant colonies default:0.25",
                        default=Defaultrho, type=np.float64)
    parser.add_argument("--q", help="quantity if pheromons deposed for ant colonies default:1",
                        default=DefaultQ, type=np.float64)
    parser.add_argument(
        "--c1", help="particle speed for PSO", default=Defaultc1, type=np.float64)
    parser.add_argument(
        "--c2", help="group speed for PSO", default=Defaultc2, type=np.float64)
    parser.add_argument(
        "--w", help="particle inertia for PSO", default=Defaultw, type=np.float64)
    parser.add_argument(
        "--t", help="duration before timeout", default=Defaulttimeout, type=int)
    ...
    if args.m not in ("ACO", "Tree_ACO", "PSO"):
        sys.exit("Error: local method must be in: [ACO, Tree_ACO, PSO]!")

    return args.i, args.n1, args.n2, args.n3, args.m, args.n, args.nt, args.alpha, args.rho, args.q, args.c1, args.c2, args.w,  args.t
...
i, n1, n2, n3, method, n, n_threads_max, alpha, rho, Q, c1, c2, w,  timeout = cmdLineParsing()
