    ...
    for j in range(nb_iteration):
        ...
        for i in range(nb_ant):
            ...
            if path in ants[0]:
                ...
                ants[0].append(path)
                ants[1].append(1)
                ants[2].append(execute(command({'filename': 'compiled/bin_'path[0]+'_'+path[1]+'.exe', 'size1':512, 'size2': 512, 'size3':512, 'num_thread':path[2], 'dim1':path[3], 'dim2': path[4], 'dim3': path[5]})))
        #update the weight
